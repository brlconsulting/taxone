CREATE OR REPLACE PACKAGE BRL_LANC_ENCERR_VAL_CPROC IS
  --
  ...
  --
END BRL_LANC_ENCERR_VAL_CPROC;
/
CREATE OR REPLACE PACKAGE BODY BRL_LANC_ENCERR_VAL_CPROC IS
  --
  -- Variaveis de status
  --
  ...
  BEGIN
    ...
    Lib_Proc.Add_Param(pstr,
                       ...
                       NULL,
                       NULL,
                       'SELECT a.cod_empresa, a.cod_empresa||'' - ''||a.razao_social FROM empresa A WHERE COD_EMPRESA = ''001'' ORDER BY a.cod_empresa');
    --
    Lib_Proc.Add_Param(pstr,
                       '___________________________________________________________________________________________________________',
                       'VARCHAR2',
  ...
  BEGIN
    RETURN 'Inclus?o Dos Lancamentos de Encerramento por Centro de Custo - SO PARA VALEANT';
  END;
  --
...
--PROCESSO DE NOVO...
    ...
    IF NUM_REG = 0 THEN
      BRL_LANC_ENCERR_VAL1_EXEC(lv_cod_empresa,
                                   num_lanc,
                                   w_DT_fim,
                                   ln_retorno);

     BRL_LANC_ENCERR_VAL2_EXEC(lv_cod_empresa,
                                   num_lanc,
                                   w_DT_fim,
                                   ln_retorno);
   ...
   END IF;
    ...
    NPROC_ID   := LIB_PROC.NEW('BRL_LANC_ENCERR_VAL_CPROC', 48, 150);
    CDESCRICAO := 'RELATORIO LANCAMENTO DE ENCERRAMENTO POR CENTRO DE CUSTO - VALEANT';
    ...
    FOR REGS IN TRANSP LOOP
      ...
      IF pTipo_Vis = 2 then
        ...
        CLINHA := LIB_STR.W(CLINHA,
                            RPAD('Valor Transportado para as duas Contas ' ||
                                 substr('*', 1, 10) || ' : ;',
                                 49,
                                 ' '),
                            1);
...
'',.'''))),
    ...
    IF pTipo_Vis = 1 THEN
      ...
      LIB_PROC.ADD(CLINHA);
      --
      CDESCRICAO := 'LANCAMENTO DE ENCERRAMENTO DO RESULTADO DO EXERCICIO';
      CLINHA     := '';
      CLINHA     := LIB_STR.W(CLINHA,
                              CDESCRICAO,
